...
module.exports = {
    ...
    async execute(client, config, interaction) {
        ...
        const channelID = config.APPLY[type].DATA.find(
            (i) => i.name == name,
        )?.CHANNEL;
        ...
        const acceptButton = new ButtonBuilder()
            .setCustomId(
                `button*_accept_${type}_${name}_${interaction.user.id}`,
            )
            .setLabel("قبول")
            .setEmoji("✔️")
        ...
        const rejectButton = new ButtonBuilder()
            .setCustomId(
                `button*_reject_${type}_${name}_${interaction.user.id}`,
            )
            .setLabel("رفض")
            .setEmoji("✖️")
